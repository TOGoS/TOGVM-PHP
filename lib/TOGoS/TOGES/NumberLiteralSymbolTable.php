...
class TOGoS_TOGES_NumberLiteralSymbolTable extends TOGoS_TOGVM_FakeArray
{
	...
	public function offsetGet($k) {
		if( preg_match('/^'.self::BOOLEAN_REGEX.'$/', $k) ) {
			return ['classRef'=>'http://ns.nuke24.net/TOGVM/Expression/LiteralBoolean', 'literalValue'=>$k == 'true'];
		} else if( preg_match('/^'.self::INTEGER_REGEX.'$/', $k) ) {
			return ['classRef'=>'http://ns.nuke24.net/TOGVM/Expression/LiteralInteger', 'literalValue'=>(int)$k];
		} else if( preg_match('/^'.self::NUMBER_REGEX.'$/', $k) ) {
			return ['classRef'=>'http://ns.nuke24.net/TOGVM/Expression/LiteralNumber', 'literalValue'=>(float)$k];
		} else {
